...
const About = () => {
    ...
    const stats = [
        { icon: fi_1.FiCode, label: 'Projects Completed', value: '50+' },
        { icon: fi_1.FiAward, label: 'Certifications', value: '15+' },
        { icon: fi_1.FiUsers, label: 'Clients Served', value: '25+' },
        { icon: fi_1.FiTrendingUp, label: 'Years Experience', value: '5+' }
    ];
    return (<section id="about" className="py-20 section-padding">
      <div className="container-width">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          {/* Left Content */}
          <framer_motion_1.motion.div initial={{ opacity: 0, x: -50 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8, delay: 0.2 }} className="space-y-6">
          ...
          {/* Right Content - Stats */}
          <framer_motion_1.motion.div initial={{ opacity: 0, x: 50 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8, delay: 0.4 }} className="grid grid-cols-2 gap-6">
            {stats.map((stat, index) => (<framer_motion_1.motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.6 + index * 0.1 }} className="bg-dark-800 p-6 rounded-xl text-center card-hover border border-dark-700">
