...
const Certifications = () => {
    ...
    return (<section id="certifications" className="py-20 section-padding bg-dark-800/50">
      <div className="container-width">
        ...
        {/* Certifications Grid */}
        <div className="mb-20">
          <framer_motion_1.motion.h3 initial={{ opacity: 0, x: -20 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 }} className="text-2xl font-semibold mb-8 text-white">
            Professional Certifications
          </framer_motion_1.motion.h3>
          
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {certifications.map((cert, index) => (<framer_motion_1.motion.div key={cert.credentialId} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.1 * index }} className="bg-dark-800 p-6 rounded-xl border border-dark-700 card-hover">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    <div className="w-12 h-12 rounded-lg overflow-hidden bg-dark-700 flex items-center justify-center">
                      <fi_1.FiAward className="text-primary-400" size={24}/>
                    </div>
                    <div>
                  ...
                  <div className="flex items-center text-gray-400 text-sm">
                    <fi_1.FiCalendar size={14} className="mr-1"/>
                    {cert.date}
                  </div>
                </div>
        ...
        {/* Education Section */}
        <div>
          <framer_motion_1.motion.h3 initial={{ opacity: 0, x: -20 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, delay: 0.4 }} className="text-2xl font-semibold mb-8 text-white">
            Education
          </framer_motion_1.motion.h3>
          
          <div className="space-y-6">
            {education.map((edu, index) => (<framer_motion_1.motion.div key={edu.degree} initial={{ opacity: 0, x: -30 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, delay: 0.5 + index * 0.1 }} className="bg-dark-800 p-6 rounded-xl border border-dark-700">
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-4">
                  ...
                  <div className="flex flex-col items-end mt-2 lg:mt-0">
                    ...
                    {edu.gpa && (<span className="text-sm text-secondary-400">
                        GPA: {edu.gpa}
                      </span>)}
                  </div>
                </div>
                
                {edu.achievements && (<div className="space-y-2">
                    <h5 className="text-sm font-medium text-gray-300 mb-2">
                      Key Achievements:
                    </h5>
                    <ul className="space-y-1">
                      {edu.achievements.map((achievement, i) => (<li key={i} className="text-gray-400 text-sm flex items-start">
