...
const Skills = () => {
    ...
    return (<section id="skills" className="py-20 section-padding bg-dark-800/50">
      <div className="container-width">
        ...
        <div className="grid lg:grid-cols-2 gap-8 mb-16">
          {skillCategories.map((category, categoryIndex) => (<framer_motion_1.motion.div key={category.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: categoryIndex * 0.1 }} className="bg-dark-800 p-6 rounded-xl border border-dark-700">
              ...
              <div className="space-y-4">
                {category.skills.map((skill, skillIndex) => (<div key={skill.name}>
                    ...
                    <div className="w-full bg-dark-700 rounded-full h-2">
                      <framer_motion_1.motion.div initial={{ width: 0 }} animate={inView ? { width: `${skill.level}%` } : {}} transition={{
                    duration: 1,
        ...
        {/* Tools & Technologies */}
        <framer_motion_1.motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, delay: 0.4 }} className="text-center">
          <h3 className="text-2xl font-semibold mb-8 text-white">
            Tools & Technologies
          </h3>
          <div className="flex flex-wrap justify-center gap-4">
            {tools.map((tool, index) => (<framer_motion_1.motion.span key={tool} initial={{ opacity: 0, scale: 0.8 }} animate={inView ? { opacity: 1, scale: 1 } : {}} transition={{ duration: 0.4, delay: 0.6 + index * 0.05 }} className="px-4 py-2 bg-dark-700 text-gray-300 rounded-full text-sm hover:bg-primary-500/20 hover:text-primary-300 transition-colors duration-300 cursor-default">
                {tool}
